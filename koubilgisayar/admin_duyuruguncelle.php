...
<div class="container content profile">
			<div class="row">
				...
				$sql = "SELECT * FROM Duyurular where ID = ".$_GET["id"];
				
				$result = $conn->query($sql);
				
				$row = $result->fetch_assoc();
				
				$DuyuruBaslik = $row["DuyuruBaslik"];
				$DuyuruIcerik = $row["DuyuruIcerikHtml"];
				...
				<div class="col-md-9">
					<div class="profile-body margin-bottom-20">
						...
						<form action="_duyuruguncelle.php" method="post">
							<div class="headline"><h3>Duyuru Ekle</h3></div>
							<div class="margin-bottom-20"></div>
							<input type="hidden" name="ID" value="<?php echo $_GET["id"] ?>" />
							<h4>Duyuru Başlığı</h4>
							<input type="text" name="DuyuruBaslik" value="<?php echo iconv("","UTF-8",$DuyuruBaslik) ?>" class="form-control"/>
							<h4>Duyuru İçeriği</h4>
							<textarea class="form-control" rows="7" name="DuyuruIcerik"><?php echo iconv("","UTF-8",$DuyuruIcerik) ?></textarea>
							<!--End Textarea-->
